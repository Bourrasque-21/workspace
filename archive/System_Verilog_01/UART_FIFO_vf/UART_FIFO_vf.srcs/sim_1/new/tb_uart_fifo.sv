    ...
    task run();
        forever begin
            ...
            $display("%t [DRV] BAUD = %0d (= %0.2f%%), BAUD_ns = %0d ns",
                     $time, uf_if.BAUD_PERIOD, uf_if.BAUD_PERIOD / 100.0,
                     uf_if.BAUD_PERIOD);

            // 약간의 여유 시간을 주어 FIFO 상태가 업데이트되게 함
            repeat (5)
            @(negedge uf_if.clk); //이것 때문에 FIFO가 full 안 나는 거라고?
    ...
    task run();
        ...
        fork
            ...
            forever begin
                ...
                $display("%t [MON_RX] RX_DATA = %2h, RX_DONE = %h", $time,
                         uf_if.rx_data, uf_if.rx_done);
...
class environment;
    ...
    task run();
        ...
        $display("**    INTERNAL pass cnt   = %8d     **",
                 scb.INTERNAL_pass_cnt);
        $display("**    INTERNAL fail cnt   = %8d     **",
                 scb.INTERNAL_fail_cnt);
